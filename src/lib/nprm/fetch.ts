...
export async function loadNprmPageData(): Promise<NprmPageData> {
  ...
  const stats = {
    ...
    // Single site-wide data timestamp = the last successful poll. The heartbeat
    // (last-check.json.pulled_at) advances on every run; stats.json.last_pull
    // only moves on a comment change, so prefer the heartbeat.
    last_pull: lastR?.data.pulled_at ?? statsR.data.last_pull,
  };
