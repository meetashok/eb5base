...
export default function ConfirmationWidget({ projectId }: ConfirmationWidgetProps) {
  ...
  return (
    <section className="bg-base-200/50 rounded-xl p-4 md:p-6">
      <h2 className="text-lg font-bold text-primary mb-4">Subscription Status</h2>
              ...
              <div key={c.id} className="flex items-center gap-2 text-sm flex-wrap">
                <span
                  className={`w-2 h-2 rounded-full shrink-0 ${
                    c.subscription_status === 'open' ? 'bg-success' : 'bg-error'
                  }`}
                />
